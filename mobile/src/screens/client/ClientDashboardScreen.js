  ...
  return (
    ...
    >
      ...
      <View className="mb-5 flex-row gap-3">
        ...
        <View className="flex-1 rounded-2xl border border-amber-500/30 bg-amber-50 dark:bg-amber-950/20 p-4">
          <View className="flex-row items-center justify-between">
            ...
            <Icon name="card" color="#d97706" size={16} />
          </View>
        ...
        <View className="flex-1 rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/90 dark:bg-slate-900/90 p-4">
          <View className="flex-row items-center justify-between">
            ...
            <Icon name="battery" color="#2563eb" size={16} />
          </View>
      ...
      <View className="mb-6 gap-2.5">
        ...
        >
          <View className="flex-row items-center gap-3">
            <View className="h-10 w-10 items-center justify-center rounded-xl bg-amber-500/15">
              <Icon name="package" color="#d97706" size={18} />
            </View>
            <View>
        ...
        {/* Stage 2: Serviced & Received Batteries — this used to link to a
            live "in service & testing" queue; clients care about what's
            been serviced and returned, which the Received bucket's
            searchable/filterable table already covers. */}
        <TouchableOpacity
          activeOpacity={0.7}
        ...
        >
          <View className="flex-row items-center gap-3">
            <View className="h-10 w-10 items-center justify-center rounded-xl bg-emerald-500/15">
              <Icon name="truck" color="#059669" size={18} />
            </View>
            <View>
              <Text className="text-sm font-bold text-slate-900 dark:text-white">Serviced & Received Batteries</Text>
              <Text className="text-[11px] text-slate-500 dark:text-slate-400">Search and filter by date</Text>
            </View>
          </View>
          <View className="flex-row items-center gap-2">
      ...
      <View className="mb-2 flex-row items-center justify-between">
        ...
        <TouchableOpacity
          onPress={() => navigation.navigate('MyBatteries', { initialBucket: 'all' })}
              ...
              <View>
                ...
                {item.serial_number ? (
                  <Text className="text-[11px] text-slate-500 dark:text-slate-400">
                    SN: {item.serial_number} {item.serial_number_added_by_role === 'client' ? '(set by you)' : ''}
                  </Text>
                ) : (
