...
function ClientBatterySortPage() {
  ...
  function handleExportExcel() {
    ...
    const exportData = activeGroup.batteries.map((code, index) => {
      ...
      return {
        '#': index + 1,
        'Battery ID': code,
        'Physical Serial Number': match?.serial_number || '—',
        'Status': isPackedForRepair
          ? 'Packed to Service'
          : match && hasBeenServiced(match)
            ...
            : match
              ? 'Not yet serviced'
              : 'Not in fleet',
        'Packed Date': isPackedForRepair && packedDate ? formatDate(packedDate) : '—',
        'Truck Number': isPackedForRepair && match?.truck_number ? match.truck_number : '—',
        'Defect Notes': match?.notes || '—',
      };
    });

    const worksheet = XLSX.utils.json_to_sheet(exportData);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Sorted Batteries');
    ...
    const safeName = (activeGroup.name || 'Sorted_Batteries').replace(/[^a-zA-Z0-9_-]/g, '_');
  ...
  const scanSuggestions = useMemo(() => {
    ...
    const existingCodes = new Set((activeGroup?.batteries || []).map((c) => c.toUpperCase()));
    return registeredBatteries
      .filter((b) => !existingCodes.has(b.battery_code.toUpperCase()))
      ...
      .slice(0, 8);
  }, [registeredBatteries, scanInput, activeGroup, NOT_WITH_CLIENT_STATUSES]);
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col gap-4 border-b border-slate-200/80 pb-5 dark:border-white/10 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-xl px-4 py-2 text-xs font-bold text-white shadow-xs transition-all hover:opacity-90 active:scale-98 ${
                hasUnsavedChanges ? 'ring-2 ring-emerald-400/50 animate-pulse' : ''
              }`}
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="h-4 w-4">
            ...
            <button
              ...
              className="rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-bold text-slate-700 shadow-2xs hover:bg-slate-100 dark:border-white/10 dark:bg-surface-850 dark:text-neutral-200 dark:hover:bg-surface-800"
            >
              Rename
            </button>
            <button
          ...
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              {showSuggestions && scanSuggestions.length > 0 && (
                <div className="absolute z-10 mt-1 w-full overflow-hidden rounded-lg border border-slate-200 bg-white shadow-lg dark:border-white/10 dark:bg-surface-850">
                  {scanSuggestions.map((b) => (
                    <button
                      key={b.id}
                      type="button"
                      onMouseDown={() => addBatteryToActiveGroup(b.battery_code)}
            ...
            <button
              ...
              className="rounded-md border border-slate-300 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 shadow-xs hover:bg-slate-50 dark:border-white/10 dark:bg-surface-850 dark:text-neutral-200 dark:hover:bg-surface-800"
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center rounded-xl border border-slate-200 bg-white p-1 shadow-2xs dark:border-white/10 dark:bg-surface-850">
            <button
              type="button"
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`inline-flex items-center gap-1 rounded-xl border px-2.5 py-1 text-xs font-extrabold shadow-2xs ${
                        hasPacked
                          ? 'border-red-300/80 bg-white/90 text-red-800 dark:border-red-500/20 dark:bg-surface-850 dark:text-red-300'
