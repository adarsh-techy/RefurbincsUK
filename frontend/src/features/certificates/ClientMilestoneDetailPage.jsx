...
function ClientMilestoneDetailPage() {
  const { id } = useParams();
  ...
  async function fetchClientMilestones() {
    setLoading(true);
    setError(null);
    try {
      const res = await apiClient.get(`/certificates/admin/client/${id}`);
      setData(res.data);
    } catch (err) {
  ...
  async function handleEvaluate() {
    ...
    setEvalMsg(null);
    try {
      const res = await apiClient.post(`/certificates/admin/evaluate/${data.client.id}`);
      const newlyCount = res.data?.newlyAwarded?.length || 0;
      if (newlyCount > 0) {
        setEvalMsg(`🎉 Awarded ${newlyCount} new milestone certificate(s)!`);
      } else {
        setEvalMsg(`✅ Milestones are up to date (${res.data?.servicedCount || 0} serviced packs).`);
      }
  ...
  if (loading) return <TableState>Loading client milestone & sustainability impact profile…</TableState>;
  ...
  if (error || !data?.client) {
    return (
      <div className="space-y-4">
        ...
        >
          <FiArrowLeft className="w-3.5 h-3.5" /> Back to Milestone Certificates
        </Link>
        <TableState tone="error">{error || 'Client milestone profile not found.'}</TableState>
      </div>
    );
  }

  const { client, servicedCount, totalCo2SavedKg, totalEwasteDivertedKg, certificates = [], nextTier, milestoneTiers = [], recentBatteries = [] } = data;
  const logo = getLogoUrl(client.logo_path);
  const co2Tons = ((servicedCount * 15.2) / 1000).toFixed(1);
  const ewasteKg = Math.round(servicedCount * 2.8).toLocaleString();

  const prevTierCount = milestoneTiers.slice().reverse().find((t) => t.count <= servicedCount)?.count || 0;
  const targetUnits = nextTier ? nextTier.count : milestoneTiers[milestoneTiers.length - 1]?.count || 100;
  ...
  const breakdownCount = servicedCount > 0 ? servicedCount : 100;
  const materialBreakdown = calculateMaterialBreakdown(breakdownCount);
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        >
          ...
          <span>Back to Milestone Certificates</span>
        </Link>

        <div className="flex items-center gap-2">
          <Link
            to={`/clients/${client.id}`}
            className="inline-flex items-center gap-1 text-xs font-bold text-slate-600 hover:text-slate-900 dark:text-neutral-400 dark:hover:text-white"
          >
            <span>View Client Operations Hub</span>
            <FiExternalLink className="w-3.5 h-3.5" />
          </Link>
        </div>
      </div>

      {/* 2. Hero Client Impact Profile Card */}
      <div className="rounded-2xl border border-slate-200/90 bg-white p-5 md:p-6 shadow-xs dark:border-white/10 dark:bg-surface-900">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          <div className="flex items-start md:items-center gap-4">
            {logo ? (
              ...
              <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-emerald-600 to-teal-700 text-white font-black text-2xl shrink-0 shadow-xs">
            ...
            <div>
              <div className="flex flex-wrap items-center gap-2.5">
                ...
                <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-0.5 text-xs font-bold text-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-400 border border-emerald-200/60 dark:border-emerald-800/40">
                  ...
                  <span>Fleet Sustainability Profile</span>
                </span>
              </div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-slate-500 dark:text-neutral-400">
                {client.login_email && (
                  <span className="inline-flex items-center gap-1.5 font-mono">
                    ...
                    <span>Login: <strong className="text-slate-700 dark:text-neutral-200">{client.login_email}</strong></span>
                  </span>
                )}
                {client.invoice_email && (
                  <span className="inline-flex items-center gap-1.5 font-mono">
                    <FiMail className="w-3.5 h-3.5 text-slate-400" />
                    <span>Billing: <strong className="text-slate-700 dark:text-neutral-200">{client.invoice_email}</strong></span>
                  </span>
                )}
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5">
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 py-2.5 text-xs font-bold text-white hover:bg-slate-800 dark:bg-surface-700 dark:hover:bg-surface-600 dark:border dark:border-white/10 disabled:opacity-50 transition-colors shadow-2xs cursor-pointer"
            >
              <FiZap className={`w-4 h-4 text-amber-400 ${isEvaluating ? 'animate-spin' : ''}`} />
              <span>{isEvaluating ? 'Evaluating…' : 'Evaluate & Award Milestones'}</span>
            </button>

            <button
              type="button"
              onClick={fetchClientMilestones}
              className="inline-flex items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2.5 text-xs font-bold text-slate-700 hover:bg-slate-50 dark:border-white/10 dark:bg-surface-800 dark:text-neutral-200 dark:hover:bg-surface-700 transition-colors shadow-2xs cursor-pointer"
            >
        ...
        {evalMsg && (
          <div className="mt-4 p-3 rounded-xl bg-emerald-50 border border-emerald-300 text-emerald-900 dark:bg-emerald-950/60 dark:border-emerald-800 dark:text-emerald-200 text-xs font-bold flex items-center justify-between">
            <span>{evalMsg}</span>
            <button type="button" onClick={() => setEvalMsg(null)} className="cursor-pointer">✕</button>
          </div>
        )}
      </div>

      {/* 3. Top Key Impact Metrics */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3.5">
        <div className="rounded-2xl border border-emerald-200/90 bg-emerald-50/50 p-4 dark:border-emerald-900/40 dark:bg-emerald-950/20 shadow-xs">
          ...
          <div className="mt-2 flex items-baseline gap-1.5">
            <span className="text-2xl font-black text-slate-900 dark:text-white">~{co2Tons}</span>
            <span className="text-xs text-emerald-700 dark:text-emerald-300 font-bold">Metric Tons</span>
          </div>
          <p className="text-[11px] text-emerald-600 dark:text-emerald-400 mt-1 font-medium">
            15.2 kg CO₂ prevented per repaired pack
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-blue-200/90 bg-blue-50/50 p-4 dark:border-blue-900/40 dark:bg-blue-950/20 shadow-xs">
          ...
          <p className="text-[11px] text-blue-600 dark:text-blue-400 mt-1 font-medium">
            Toxic landfill contamination prevented
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-purple-200/90 bg-purple-50/50 p-4 dark:border-purple-900/40 dark:bg-purple-950/20 shadow-xs">
          ...
          <div className="mt-2 flex items-baseline gap-1.5">
            <span className="text-2xl font-black text-slate-900 dark:text-white">{certificates.length}</span>
      ...
      {/* 5. 6-Tier Milestone Roadmap */}
      <div className="space-y-3">
        <div>
          <h2 className="text-base font-black text-slate-900 dark:text-white">
            Client Milestone Roadmap
          </h2>
      ...
      {/* 6. Awarded Milestone Certificates Collection */}
      <div className="mt-8 pt-6 border-t border-slate-200/80 dark:border-white/10 space-y-4">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-base sm:text-lg font-black text-slate-900 dark:text-white">
              Awarded Milestone Certificates ({certificates.length})
            </h2>
            <p className="text-xs text-slate-500 dark:text-neutral-400">
              Verified ESG sustainability certificates awarded to this fleet client.
            </p>
          </div>
        </div>
        ...
        {certificates.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-200 p-8 sm:p-10 text-center bg-white dark:border-white/10 dark:bg-surface-900 space-y-3">
            ...
            <h3 className="text-base font-bold text-slate-900 dark:text-white">
              No Milestone Certificates Earned Yet
            </h3>
            <p className="text-xs text-slate-500 dark:text-neutral-400 max-w-md mx-auto">
              This client has serviced {servicedCount} batteries. The first Bronze Milestone Certificate unlocks automatically once 100 units are serviced.
            </p>
          </div>
        ) : (
      ...
      {/* 7. Recent Serviced Batteries Contributing to Milestones */}
      {recentBatteries.length > 0 && (
        <div className="rounded-2xl border border-slate-200 bg-white p-5 dark:border-white/10 dark:bg-surface-900 shadow-xs space-y-3">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3 dark:border-white/5">
            <div className="flex items-center gap-2">
              <FiBatteryCharging className="w-4 h-4 text-emerald-500" />
              <h3 className="text-sm font-bold text-slate-900 dark:text-white">
                Recent Serviced Batteries for {client.name} ({recentBatteries.length})
              </h3>
            </div>
          </div>
          ...
          <div className="overflow-x-auto rounded-xl border border-slate-200/80 dark:border-white/10">
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50 border-b border-slate-200 dark:bg-surface-800 dark:border-white/10">
                <tr>
                  <th className="px-3.5 py-2.5 font-bold text-slate-700 dark:text-neutral-300">Battery Code</th>
                  <th className="px-3.5 py-2.5 font-bold text-slate-700 dark:text-neutral-300">Serial Number</th>
                  <th className="px-3.5 py-2.5 font-bold text-slate-700 dark:text-neutral-300">Status</th>
                  <th className="px-3.5 py-2.5 font-bold text-slate-700 dark:text-neutral-300">Last Serviced Date</th>
                  <th className="px-3.5 py-2.5 text-right font-bold text-slate-700 dark:text-neutral-300">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100 dark:divide-white/5">
                {recentBatteries.map((b) => (
                  <tr key={b.id} className="hover:bg-slate-50/70 dark:hover:bg-white/5 transition-colors">
                    <td className="px-3.5 py-2.5 font-mono font-bold text-blue-600 dark:text-blue-400">
                      {b.battery_code}
                    </td>
                    <td className="px-3.5 py-2.5 font-mono text-slate-600 dark:text-neutral-300">
                      {b.serial_number || '—'}
                    </td>
                    <td className="px-3.5 py-2.5">
                      <StatusBadge status={b.status} />
                    </td>
                    <td className="px-3.5 py-2.5 text-slate-500 dark:text-neutral-400">
                      {b.last_repaired_at
                        ? new Date(b.last_repaired_at).toLocaleDateString([], { day: '2-digit', month: 'short', year: 'numeric' })
                        : new Date(b.created_at).toLocaleDateString([], { day: '2-digit', month: 'short', year: 'numeric' })}
                    </td>
                    <td className="px-3.5 py-2.5 text-right">
                      <Link
                        to={`/batteries/${encodeURIComponent(b.battery_code)}`}
      ...
      {/* 8. Technical Chemical Recovery Annex (Collapsible) */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-bold text-sm text-slate-900 dark:text-white">
                Technical Annex: 14-Element Chemical Recovery Audit ({breakdownCount.toLocaleString()} Packs)
              </h3>
              <p className="text-xs text-slate-500 dark:text-neutral-400">
                Audited metallurgical mass breakdown for ISO 14001, RoHS, and EU Battery Passport compliance.
              </p>
            </div>
          </div>
...
export default ClientMilestoneDetailPage;
