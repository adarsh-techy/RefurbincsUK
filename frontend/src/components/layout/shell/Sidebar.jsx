...
const RECYCLE_CLIENT_NAV_GROUPS = [
  ...
  },
  {
    heading: 'Logistics',
    links: [
      { to: '/my/recycle-shipments', label: 'Recycle Shipments', icon: Icons.recycle },
    ],
  },
  {
    heading: 'Settings',
...
const NAV_GROUPS = [
  {
    ...
    links: [
      ...
      { to: '/messages', label: 'Support Messages', icon: Icons.messages },
    ],
  },
  {
...
function SidebarNav({ visibleGroups, user, onLinkClick, onLogout, customTheme }) {
  ...
  const isAccentBg = !isDark && !!customTheme?.applyToSidebar;
  ...
  const brandLogo = isDark || isAccentBg ? refurbnicsDarkLogo : refurbnicsLightLogo;
          ...
          {clientLogo ? (
            ...
            <Link to="/" className="flex items-center">
              <img
                src={brandLogo}
                alt="Refurbinics"
                className="max-h-12 max-w-[200px] w-auto object-contain transition-transform hover:scale-102"
              />
            </Link>
          )}
        </div>
                          ...
                          className={`mr-3 shrink-0 transition-colors ${
                            isAccentBg
                              ...
                              : isActive
                                ...
                                : 'text-slate-400 group-hover:text-slate-600 dark:text-neutral-400 dark:group-hover:text-neutral-200'
                          }`}
                        >
                          {link.icon}
                        </span>
...
function Sidebar({ mobileOpen = false, onClose = () => {} }) {
  ...
  function handleLogout() {
    onClose();
    dispatch(logout());
    // /batteries/:code renders outside ProtectedRoute (it's also reachable
    // via public QR scan), so logging out there doesn't trigger a route
    // guard redirect on its own — navigate explicitly instead.
  ...
  const sidebarBgColor = isAccentBg
  ...
  return (
    <>
      <aside
        className={`fixed inset-y-0 left-0 z-30 hidden h-screen w-64 shrink-0 flex-col md:flex transition-all shadow-md ${
          isAccentBg ? '' : 'dark:border-r dark:border-white/10'
        }`}
      ...
      {createPortal(
        ...
        >
          ...
          <aside
            className="absolute inset-y-0 left-0 flex w-72 max-w-[85vw] flex-col shadow-2xl transition-transform duration-300 touch-scroll pb-safe"
            style={{
          ...
          >
            <button
              ...
              className={`absolute right-3 top-3 z-10 rounded-xl p-2 min-h-[44px] min-w-[44px] flex items-center justify-center ${
