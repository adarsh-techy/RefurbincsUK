...
function DataTable({
  ...
  return (
    ...
    >
      <table
        className={`w-full text-sm ${isFixed ? 'table-fixed' : 'min-w-full'} ${
      ...
      >
        <thead className={header.thead}>
          <tr className={bordered ? `divide-x ${header.divide}` : ''}>
            {showRowNumber && (
              <th
                style={{ width: '48px' }}
                className={`w-12 shrink-0 whitespace-nowrap px-4 py-3.5 text-left text-sm font-bold uppercase tracking-wide ${header.headText}`}
              >
                #
              </th>
            )}
            {columns.map((col) => {
              ...
              return (
                <th
                  ...
                  className={`px-5 py-3.5 text-left text-sm font-bold uppercase tracking-wide transition-colors ${
                ...
                >
                  <div className="inline-flex items-center gap-1.5 max-w-full truncate">
                    ...
                    {isSortable && (
                      <span className="inline-flex flex-col text-[11px] leading-none shrink-0">
                        {isSorted ? (
                          sortDirection === 'asc' ? (
        ...
        <tbody className={bordered ? `divide-y ${header.tbodyDivide}` : ''}>
          {sortedRows.map((row, i) => (
            ...
            >
              {showRowNumber && (
                <td className="w-12 whitespace-nowrap px-4 py-3.5 text-slate-500 dark:text-neutral-400">{i + 1}</td>
              )}
              {columns.map((col) => (
                <td
                  key={col.key}
                  style={col.width ? { width: col.width } : undefined}
                  className={`px-5 py-3.5 text-slate-700 dark:text-neutral-200 ${
